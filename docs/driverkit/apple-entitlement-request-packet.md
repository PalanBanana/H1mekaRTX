...
- Research and development of a read-only DriverKit PCI probe path for RTX 5070 hardware visibility and activation plumbing on macOS.
...
The current probe does not write MMIO, access BAR memory, reset the GPU, initialize firmware, initialize display engines, provide framebuffer support, provide Metal support, or attempt graphics acceleration.

## Safety Boundary

This project stage does not attempt:
...
- Confirm Team ID
- Confirm Host App bundle identifier
- Confirm DriverKit extension bundle identifier
- Confirm requested entitlements
- Confirm repository URL
- Confirm RTX 5070 PCI identity values
- Confirm read-only safety boundary
