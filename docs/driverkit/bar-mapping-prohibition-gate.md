# BAR Mapping Prohibition Gate

## Purpose

This Phase 28 contract defines a BAR mapping prohibition gate for H1mekaRTX.

This phase blocks BAR mapping and MMIO access paths before any real DriverKit / PCIDriverKit hardware aperture interaction is allowed.
...
This phase audits runtime activation and provider-candidate artifacts for forbidden BAR mapping, memory descriptor, configuration write, MMIO write, command submission, firmware, reset, framebuffer, and display-engine initialization markers.
...
All Phase 28 outputs must be labeled only as:
...
The checker constructs forbidden runtime marker strings from parts at runtime.
...
This avoids false positives from the existing Stage 8 repo audit while still preserving the prohibition gate.
...
Static checker scripts, docs, and generated reports are intentionally excluded from marker scanning because they may contain explanatory blocked hardware-access wording without creating an executable hardware-access path.
...
2. BAR mapping remains forbidden.
3. MMIO mutation remains forbidden.
4. PCI configuration writes remain forbidden.
5. Memory descriptor mapping remains forbidden.
6. GPU command submission remains forbidden.
7. Firmware load remains forbidden.
8. GPU reset remains forbidden.
9. Framebuffer initialization remains forbidden.
10. Display-engine initialization remains forbidden.
...
BAR mapping prohibition is not Dock, transparency, blur, Mission Control, Launchpad, Stage Manager, UI compositor, or Metal acceleration proof.
...
- PHASE28_BAR_MAPPING_PROHIBITION_GATE_READY: True
- BAR_MAPPING_PROHIBITION_GATE_ONLY: True
- BAR_MAPPING_FORBIDDEN: True
- BAR_MMIO_MUTATION_FORBIDDEN: True
- CONFIGURATION_WRITES_FORBIDDEN: True
