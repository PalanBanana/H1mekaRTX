...
SCHEMA = "h1mekartx.entitlement_resolver_fixture_matrix_check.v1"

REQUIRED_FILES = [
    "scripts/validate-entitlement-resolver-fixture-matrix.py",
    "scripts/check-entitlement-resolver-fixture-matrix.py",
    "docs/metal/entitlement-resolver-fixture-matrix.md",
    "evidence-templates/entitlement-evidence.runtime-requested.fixture.json",
]

REQUIRED_TERMS = [
    "PASS_ENTITLEMENT_RESOLVER_FIXTURE_MATRIX",
    ...
    "evidence_file_attempts_to_enable_runtime",
...
def run_validator(root: Path, out_dir: Path) -> dict[str, Any]:
    proc = subprocess.run(
        [
            "python3",
            str(root / "scripts" / "validate-entitlement-resolver-fixture-matrix.py"),
            "--root",
            str(root),
            "--out-dir",
            str(out_dir),
...
def build_report(root: Path, out_dir: Path) -> dict[str, Any]:
    ...
    runtime_fixture_path = root / "evidence-templates/entitlement-evidence.runtime-requested.fixture.json"
    runtime_fixture = json.loads(runtime_fixture_path.read_text()) if runtime_fixture_path.exists() else {}
    runtime_evidence = runtime_fixture.get("evidence", {}) if isinstance(runtime_fixture, dict) else {}
    if not isinstance(runtime_evidence, dict):
        runtime_evidence = {}

    add_check(checks, "runtime_fixture_schema", runtime_fixture.get("schema") == "h1mekartx.entitlement_evidence_input.v1", f"schema={runtime_fixture.get('schema')!r}")
    add_check(checks, "runtime_fixture_claims_runtime_true", runtime_evidence.get("metal_injection_runtime_allowed_now") is True, f"value={runtime_evidence.get('metal_injection_runtime_allowed_now')!r}")
    add_check(checks, "runtime_fixture_claims_rtx_runtime_true", runtime_evidence.get("rtx5070_metal_runtime_allowed") is True, f"value={runtime_evidence.get('rtx5070_metal_runtime_allowed')!r}")
    ...
    report_path = out_dir / "entitlement-resolver-fixture-matrix-report.json"
    report = json.loads(report_path.read_text()) if report_path.exists() else {}

    add_check(checks, "matrix_schema", report.get("schema") == "h1mekartx.entitlement_resolver_fixture_matrix.v1", f"schema={report.get('schema')!r}")
    add_check(checks, "matrix_decision", report.get("decision") == "PASS_ENTITLEMENT_RESOLVER_FIXTURE_MATRIX", f"decision={report.get('decision')!r}")
    ...
    add_check(checks, "sample_fixture_no_go", sample.get("decision") == "NO_GO_ENTITLEMENT_EVIDENCE_INCOMPLETE", f"decision={sample.get('decision')!r}")
    add_check(checks, "ready_fixture_manual_review", ready.get("gate") == "GO_EVIDENCE_PRESENT_FOR_MANUAL_REVIEW_NO_RUNTIME", f"gate={ready.get('gate')!r}")
    add_check(checks, "ready_fixture_runtime_false", ready.get("runtime_allowed_after_resolver") is False, f"value={ready.get('runtime_allowed_after_resolver')!r}")
    add_check(checks, "runtime_requested_fixture_no_go", runtime_requested.get("decision") == "NO_GO_ENTITLEMENT_EVIDENCE_INCOMPLETE", f"decision={runtime_requested.get('decision')!r}")
    add_check(checks, "runtime_requested_fixture_runtime_false", runtime_requested.get("runtime_allowed_after_resolver") is False, f"value={runtime_requested.get('runtime_allowed_after_resolver')!r}")
    ...
    return {
        ...
        "repo_root": str(root),
        "decision": "PASS_ENTITLEMENT_RESOLVER_FIXTURE_MATRIX_READY" if failed_count == 0 else "FAIL_ENTITLEMENT_RESOLVER_FIXTURE_MATRIX",
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Entitlement Resolver Fixture Matrix Check",
            "",
            ...
            *rows,
            ...
            "## Safety Boundary",
            "",
            "This check validates resolver fixture matrix coverage only. It adds no driver installation, no driver activation, no provider transition, no device ownership transition, no hardware-path code, and no RTX 5070 Metal runtime.",
            "",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check entitlement resolver fixture matrix.")
    ...
    json_path = out_dir / "entitlement-resolver-fixture-matrix-check.json"
    md_path = out_dir / "entitlement-resolver-fixture-matrix-check.md"
