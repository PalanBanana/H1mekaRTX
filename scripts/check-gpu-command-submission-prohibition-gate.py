...
SCHEMA = "h1mekartx.gpu_command_submission_prohibition_gate_check.v1"
...
REQUIRED_CONTRACT_TOKENS = [
    ...
    "GPU_COMMAND_SUBMISSION_PROHIBITION_GATE_ONLY: True",
    ...
    "GSP_FIRMWARE_LOAD_FORBIDDEN: True",
    ...
    "GPU_COMMAND_SUBMISSION_PROHIBITION_AUDIT_SCOPE",
    "GPU_COMMAND_SUBMISSION_PROHIBITION_GATE_RULE",
...
GPU_COMMAND_MARKERS = ["".join(parts) for parts in GPU_COMMAND_MARKER_PARTS]
FIRMWARE_RESET_MARKERS = ["".join(parts) for parts in FIRMWARE_RESET_MARKER_PARTS]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check GPU command submission prohibition gate.")
    ...
    contract_path = root / "docs" / "driverkit" / "gpu-command-submission-prohibition-gate.md"
    gate_path = root / "tools" / "driverkit-activation" / "gpu-command-submission-prohibition-gate.json"
    ...
    checks = [
        ...
        make_check("gpu_command_gate_json_exists", gate_path.exists(), str(gate_path)),
    ...
    checks.append(make_check("gate_schema_matches", bool(gate and gate.get("schema") == "h1mekartx.gpu_command_submission_prohibition_gate.v1"), "gate schema"))
    checks.append(make_check("gate_gpu_command_forbidden", bool(gate and gate.get("gpu_command_submission_forbidden") is True), "GPU command submission forbidden"))
    checks.append(make_check("gate_state_enforced", bool(gate and gate.get("gpu_command_submission_prohibition_state") == "ENFORCED"), "ENFORCED"))
    ...
    for rel in AUDIT_SCOPE:
        ...
        if not p.exists():
            ...
            continue
        gpu_command_hits.extend(scan_file(p, GPU_COMMAND_MARKERS, "gpu_command_submission"))
        firmware_reset_hits.extend(scan_file(p, FIRMWARE_RESET_MARKERS, "firmware_reset_or_display_init"))

    checks.append(make_check("audit_scope_files_exist", not missing_scope, ",".join(missing_scope)))
    checks.append(make_check("no_forbidden_gpu_command_markers", not gpu_command_hits, json.dumps(gpu_command_hits, sort_keys=True)))
    checks.append(make_check("no_forbidden_firmware_reset_markers", not firmware_reset_hits, json.dumps(firmware_reset_hits, sort_keys=True)))
    ...
    decision = "PASS_GPU_COMMAND_SUBMISSION_PROHIBITION_GATE_READY" if failed_count == 0 else "FAIL_GPU_COMMAND_SUBMISSION_PROHIBITION_GATE"
    ...
    report = {
        ...
        "secondary_classification": "CLASSIFICATION_CONFIG_WRITE_PROHIBITION_GATE",
        ...
        "bar_mapping_prohibition_state": "ENFORCED",
        "provider_open_prohibition_state": "ENFORCED",
        "gpu_command_hits": gpu_command_hits,
        "firmware_reset_hits": firmware_reset_hits,
        "provider_match_candidate_summary_state": "SUMMARY_ONLY",
        "provider_match_proof_state": "NOT_ATTEMPTED",
        "dext_load_proof_state": "NOT_ATTEMPTED",
        "real_gpu_command_execution_proof_state": "NOT_ATTEMPTED",
    ...
    json_path = out_dir / "gpu-command-submission-prohibition-gate-check.json"
    ...
    md = f"""# GPU Command Submission Prohibition Gate Check
...
- GPU Queue Creation Forbidden: `{report['gpu_queue_creation_forbidden']}`
- GPU Command Buffer Creation Forbidden: `{report['gpu_command_buffer_creation_forbidden']}`
- GPU Encoder Creation Forbidden: `{report['gpu_encoder_creation_forbidden']}`
- GPU Command Commit Forbidden: `{report['gpu_command_commit_forbidden']}`
- GSP Firmware Load Forbidden: `{report['gsp_firmware_load_forbidden']}`
...
- GPU Command Hits: `{len(gpu_command_hits)}`
- Firmware / Reset / Display Init Hits: `{len(firmware_reset_hits)}`
...
- GPU Command Queue Attempted: `{report['gpu_command_queue_attempted']}`
- GPU Command Buffer Attempted: `{report['gpu_command_buffer_attempted']}`
- GPU Command Encoder Attempted: `{report['gpu_command_encoder_attempted']}`
- GPU Command Commit Attempted: `{report['gpu_command_commit_attempted']}`
...
"""
    md_path = out_dir / "gpu-command-submission-prohibition-gate-check.md"
