...
manifest_path = ROOT / "tools/hackintosh/redacted-bundle-assembly-dry-run-consistency-gate.json"
doc_path = ROOT / "docs/hackintosh/redacted-bundle-assembly-dry-run-consistency-gate.md"
out_path = ROOT / "release-readiness/redacted-bundle-assembly-dry-run-consistency-gate.json"
out_md_path = ROOT / "release-readiness/redacted-bundle-assembly-dry-run-consistency-gate.md"
...
add(checks, "manifest_schema", bool(manifest and manifest.get("schema") == "h1mekartx.redacted_bundle_assembly_dry_run_consistency_gate.v1"), "manifest schema")
add(checks, "output_schema", bool(out and out.get("schema") == "h1mekartx.redacted_bundle_assembly_dry_run_consistency_gate_output.v1"), "output schema")
add(checks, "output_decision_pass", bool(out and out.get("decision") == "PASS_REDACTED_BUNDLE_ASSEMBLY_DRY_RUN_CONSISTENCY_GATE_READY"), "output decision")
add(checks, "output_ready_true", bool(out and out.get("redacted_bundle_assembly_dry_run_consistency_gate_ready") is True), "ready")
add(checks, "output_assembly_entries_present_true", bool(out and out.get("assembly_entries_present") is True), "assembly")
add(checks, "output_assembly_count_matches_true", bool(out and out.get("assembly_count_matches") is True), "assembly")
add(checks, "output_assembly_all_sources_exist_true", bool(out and out.get("assembly_all_sources_exist") is True), "assembly")
add(checks, "output_assembly_all_source_paths_relative_true", bool(out and out.get("assembly_all_source_paths_relative") is True), "assembly")
add(checks, "output_assembly_all_bundle_paths_relative_true", bool(out and out.get("assembly_all_bundle_paths_relative") is True), "assembly")
add(checks, "output_assembly_forbidden_absent_true", bool(out and out.get("assembly_forbidden_absent") is True), "assembly")
add(checks, "output_assembly_all_not_copied_true", bool(out and out.get("assembly_all_not_copied") is True), "assembly")
add(checks, "output_assembly_sha256_present_true", bool(out and out.get("assembly_sha256_present") is True), "assembly")
add(checks, "output_private_paths_absent_true", bool(out and out.get("private_paths_absent") is True), "private paths")
...
for field in [
    "input_assembly_manifest_pass",
    "input_assembly_manifest_check_pass",
    "input_assembly_manifest_ready",
    "input_inventory_consistency_gate_ready",
    "input_inventory_ledger_ready",
    "inputs_safe",
]:
...
decision = "PASS_REDACTED_BUNDLE_ASSEMBLY_DRY_RUN_CONSISTENCY_GATE_READY" if failed == 0 else "FAIL_REDACTED_BUNDLE_ASSEMBLY_DRY_RUN_CONSISTENCY_GATE"

report = {
    "schema": "h1mekartx.redacted_bundle_assembly_dry_run_consistency_gate_check.v1",
...
(OUT / "redacted-bundle-assembly-dry-run-consistency-gate-check.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")

rows = "\n".join(f"| `{c['name']}` | {'PASS' if c['passed'] else 'FAIL'} | {c['detail']} |" for c in checks)
(OUT / "redacted-bundle-assembly-dry-run-consistency-gate-check.md").write_text(f"""# Redacted Bundle Assembly Dry-Run Consistency Gate Check
