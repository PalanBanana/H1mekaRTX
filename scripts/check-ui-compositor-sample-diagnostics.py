...
SCHEMA = "h1mekartx.ui_compositor_sample_diagnostics_check.v1"

REQUIRED_FILES = [
    "scripts/collect-ui-compositor-sample.sh",
    "scripts/summarize-ui-compositor-sample.py",
    "docs/metal/ui-compositor-sample-diagnostics.md",
]
...
def write_fixture(input_dir: Path) -> None:
    ...
    files = {
        "sample-metadata.json": json.dumps(
            {
                ...
                "label": "fixture-dock-window-mission-control",
                "logWindow": "5m",
            },
            indent=2,
            sort_keys=True,
        ...
        "uptime.txt": "10:00 up 1 day, 1 user, load averages: 1.0 1.0 1.0\n",
        "system_profiler-SPDisplaysDataType.txt": "Displays:\n  Metal Support: Supported\n",
        "system_profiler-SPDisplaysDataType.json": json.dumps({"SPDisplaysDataType": []}) + "\n",
        ...
        "ui-preferences.txt": "AppleReduceTransparency = 0\nAppleReduceMotion = 0\nmagnification = 1\n",
        "ioreg-graphics-filtered.txt": "IODisplay IOFramebuffer IOGraphics IOAccelerator Metal GPU display accelerator\n",
        "logs-ui-compositor-window.txt": "WindowServer Dock CoreAnimation QuartzCore Metal IOAccelerator IODisplay IOFramebuffer Mission Control Launchpad blur backdrop vibrancy\n",
...
def run_summarizer(root: Path, input_dir: Path, out_dir: Path) -> dict[str, Any]:
    proc = subprocess.run(
        [
            "python3",
            str(root / "scripts" / "summarize-ui-compositor-sample.py"),
            "--input-dir",
            str(input_dir),
            "--out-dir",
            str(out_dir),
...
def build_report(root: Path, out_dir: Path) -> dict[str, Any]:
    ...
    required_terms = [
        "h1mekartx.ui_compositor_sample_summary.v1",
    ...
    fixture_dir = out_dir / "fixture" / "ui-compositor-sample"
    ...
    json_path = summary_out / "ui-compositor-sample-summary.json"
    md_path = summary_out / "ui-compositor-sample-summary.md"
    ...
    add("summary_schema", data.get("schema") == "h1mekartx.ui_compositor_sample_summary.v1", f"schema={data.get('schema')!r}")
    add("sample_result", data.get("decisions", {}).get("sample_result") == "UI_COMPOSITOR_SAMPLE_DIAGNOSTICS_CAPTURED", f"result={data.get('decisions', {}).get('sample_result')!r}")
    ...
    for key in ["read_only", "runs_log_show", "runs_ioreg", "runs_system_profiler"]:
    ...
    return {
        ...
        "repo_root": str(root),
        "decision": "PASS_UI_COMPOSITOR_SAMPLE_DIAGNOSTICS_READY" if failed_count == 0 else "FAIL_UI_COMPOSITOR_SAMPLE_DIAGNOSTICS_NOT_READY",
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# UI Compositor Sample Diagnostics Check",
            "",
            ...
            *rows,
            ...
            "## Safety Boundary",
            "",
            "This check validates UI compositor sample diagnostics using local fixtures only. It does not activate DriverKit or System Extensions, inject into WindowServer or Dock, patch private frameworks, bypass SIP/AMFI, write PCI config space, access MMIO, map BAR memory, submit GPU commands, prove UI compositor acceleration, or prove Metal acceleration.",
            "",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check H1mekaRTX UI compositor sample diagnostics.")
    ...
    json_path = out_dir / "ui-compositor-sample-diagnostics-check.json"
    md_path = out_dir / "ui-compositor-sample-diagnostics-check.md"
