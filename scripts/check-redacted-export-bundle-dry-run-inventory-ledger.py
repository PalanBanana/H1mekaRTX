...
manifest_path = ROOT / "tools/hackintosh/redacted-export-bundle-dry-run-inventory-ledger.json"
doc_path = ROOT / "docs/hackintosh/redacted-export-bundle-dry-run-inventory-ledger.md"
out_path = ROOT / "release-readiness/redacted-export-bundle-dry-run-inventory-ledger.json"
out_md_path = ROOT / "release-readiness/redacted-export-bundle-dry-run-inventory-ledger.md"
...
add(checks, "manifest_schema", bool(manifest and manifest.get("schema") == "h1mekartx.redacted_export_bundle_dry_run_inventory_ledger.v1"), "manifest schema")
add(checks, "output_schema", bool(out and out.get("schema") == "h1mekartx.redacted_export_bundle_dry_run_inventory_ledger_output.v1"), "output schema")
add(checks, "output_decision_pass", bool(out and out.get("decision") == "PASS_REDACTED_EXPORT_BUNDLE_DRY_RUN_INVENTORY_LEDGER_READY"), "output decision")
add(checks, "output_ready_true", bool(out and out.get("redacted_export_bundle_dry_run_inventory_ledger_ready") is True), "ready")
...
inventory = out.get("inventory", []) if isinstance(out, dict) else []
add(checks, "output_inventory_count_matches", bool(out and out.get("inventory_count") == len(inventory) and len(inventory) >= 3), "inventory count")
add(checks, "output_inventory_sha256_present", bool(inventory and all(i.get("sha256") for i in inventory)), "sha256")
...
for field in [
    "input_plan_consistency_gate_pass",
    "input_plan_consistency_gate_check_pass",
...
decision = "PASS_REDACTED_EXPORT_BUNDLE_DRY_RUN_INVENTORY_LEDGER_READY" if failed == 0 else "FAIL_REDACTED_EXPORT_BUNDLE_DRY_RUN_INVENTORY_LEDGER"

report = {
    "schema": "h1mekartx.redacted_export_bundle_dry_run_inventory_ledger_check.v1",
...
(OUT / "redacted-export-bundle-dry-run-inventory-ledger-check.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")

rows = "\n".join(f"| `{c['name']}` | {'PASS' if c['passed'] else 'FAIL'} | {c['detail']} |" for c in checks)
(OUT / "redacted-export-bundle-dry-run-inventory-ledger-check.md").write_text(f"""# Redacted Export Bundle Dry-Run Inventory Ledger Check
