...
echo "== H1mekaRTX Phase 29 PCI Configuration Write Prohibition Gate Verification =="
...
echo "== 3. Run PCI configuration write prohibition gate checker =="
python3 "${ROOT_DIR}/scripts/check-config-write-prohibition-gate.py" \
  --root "${ROOT_DIR}" \
  --out-dir "${OUT_DIR}"
...
echo "== Phase 29 PCI configuration write prohibition gate verification passed =="
