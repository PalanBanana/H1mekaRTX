...
echo "== H1mekaRTX Phase 10 Static Build/Signing/Entitlement Audit Verification =="
...
echo "== 3. Run static build/signing/entitlement audit checker =="
python3 "${ROOT_DIR}/scripts/check-static-build-signing-entitlement-audit.py" \
  --root "${ROOT_DIR}" \
  --out-dir "${OUT_DIR}"
...
echo "== Phase 10 static build/signing/entitlement audit verification passed =="
