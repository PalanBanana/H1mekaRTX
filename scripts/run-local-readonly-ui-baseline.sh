...
echo "== H1mekaRTX Phase 36 Local Read-Only UI Baseline Collector Verification =="
...
echo "== 3. Collect local read-only UI baseline =="
python3 "${ROOT_DIR}/scripts/collect-local-readonly-ui-baseline.py" \
  --root "${ROOT_DIR}" \
  --out-dir "host-report-bundle/ui-baseline"

echo "== 4. Run local read-only UI baseline checker =="
python3 "${ROOT_DIR}/scripts/check-local-readonly-ui-baseline.py" \
  --root "${ROOT_DIR}" \
  --out-dir "${OUT_DIR}"
...
echo "== Phase 36 local read-only UI baseline collector verification passed =="
