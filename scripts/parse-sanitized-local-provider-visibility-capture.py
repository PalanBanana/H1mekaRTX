...
RAW_PATH = ROOT / "host-report-bundle/readonly-provider-visibility/readonly-provider-visibility-local-capture.json"
OPTIN_ENV = "H1MEKARTX_PARSE_LOCAL_PROVIDER_VISIBILITY_CAPTURE"
OPTIN_VALUE = "I_UNDERSTAND_SANITIZED_READONLY_PROVIDER_VISIBILITY_PARSE_ONLY"
...
def command_summary(cmd: dict) -> dict:
    stdout = scrub_text(str(cmd.get("stdout_tail", "")))
    stderr = scrub_text(str(cmd.get("stderr_tail", "")))
    combined = (stdout + "\n" + stderr).lower()
    return {
        "name": cmd.get("name"),
...
summary = {
    "schema": "h1mekartx.sanitized_local_provider_visibility_capture_parser_summary.v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "classification": "CLASSIFICATION_SANITIZED_LOCAL_PROVIDER_VISIBILITY_CAPTURE_PARSER_SUMMARY",
    "rtx5070_target_retained": True,
    "fallback_gpu_substitution_allowed": False,
    "hard_opt_in_required": True,
    "hard_opt_in_matched": hard_opt_in,
    "raw_capture_exists_local": raw_exists,
    "raw_capture_read": bool(raw is not None),
    "raw_stdout_committed": False,
    "raw_stderr_committed": False,
...
if not hard_opt_in:
    summary["decision"] = "REFUSED_SANITIZED_PROVIDER_VISIBILITY_PARSE_MISSING_HARD_OPTIN"
elif not raw_exists:
    summary["decision"] = "NO_LOCAL_PROVIDER_VISIBILITY_CAPTURE_PRESENT"
else:
    cmds = raw.get("commands", []) if isinstance(raw, dict) else []
    parsed_cmds = [command_summary(c) for c in cmds if isinstance(c, dict)]
    summary.update({
        "decision": "PASS_SANITIZED_PROVIDER_VISIBILITY_CAPTURE_PARSED",
        "raw_decision": raw.get("decision"),
        "raw_hard_opt_in_matched": raw.get("hard_opt_in_matched"),
        "raw_commands_executed": raw.get("commands_executed"),
        "command_count": len(parsed_cmds),
        "commands": parsed_cmds,
        "detected_any_h1mekartx": any(c["mentions_h1mekartx"] for c in parsed_cmds),
        "detected_any_iopcidevice": any(c["mentions_iopcidevice"] for c in parsed_cmds),
        "detected_any_10de": any(c["mentions_10de"] for c in parsed_cmds),
        "detected_any_2f04": any(c["mentions_2f04"] for c in parsed_cmds),
        "detected_any_2f0410de": any(c["mentions_2f0410de"] for c in parsed_cmds),
    })

json_path = OUT / "sanitized-local-provider-visibility-capture-parser-summary.json"
md_path = OUT / "sanitized-local-provider-visibility-capture-parser-summary.md"
json_path.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")

cmd_lines = "\n".join(
    f"- `{c.get('name')}` rc=`{c.get('returncode')}` h1mekartx=`{c.get('mentions_h1mekartx')}` iopcidevice=`{c.get('mentions_iopcidevice')}` 10de=`{c.get('mentions_10de')}` 2f04=`{c.get('mentions_2f04')}`"
    for c in summary.get("commands", [])
) or "- none"

md_path.write_text(f"""# Sanitized Local Provider Visibility Capture Parser Summary
