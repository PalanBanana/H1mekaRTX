...
manifest_path = ROOT / "tools/hackintosh/entitlement-request-evidence-checklist.json"
doc_path = ROOT / "docs/hackintosh/entitlement-request-evidence-checklist.md"
out_path = ROOT / "release-readiness/entitlement-request-evidence-checklist.json"
out_md_path = ROOT / "release-readiness/entitlement-request-evidence-checklist.md"
...
add(checks, "manifest_schema", bool(manifest and manifest.get("schema") == "h1mekartx.entitlement_request_evidence_checklist.v1"), "manifest schema")
add(checks, "output_schema", bool(out and out.get("schema") == "h1mekartx.entitlement_request_evidence_checklist_output.v1"), "output schema")
add(checks, "output_decision_pass", bool(out and out.get("decision") == "PASS_ENTITLEMENT_REQUEST_EVIDENCE_CHECKLIST_READY"), "output decision")
...
for field in [
    "input_package_skeleton_pass",
    "input_package_skeleton_check_pass",
    "input_package_skeleton_ready",
    "input_handoff_ready",
    "request_notes_template_ready",
    "inputs_safe",
    "entitlement_request_evidence_checklist_ready",
]:
...
add(checks, "output_blocked_count_at_least_three", bool(out and out.get("blocked_count", 0) >= 3), "blocked_count")
...
decision = "PASS_ENTITLEMENT_REQUEST_EVIDENCE_CHECKLIST_READY" if failed == 0 else "FAIL_ENTITLEMENT_REQUEST_EVIDENCE_CHECKLIST"

report = {
    "schema": "h1mekartx.entitlement_request_evidence_checklist_check.v1",
...
(OUT / "entitlement-request-evidence-checklist-check.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")

rows = "\n".join(f"| `{c['name']}` | {'PASS' if c['passed'] else 'FAIL'} | {c['detail']} |" for c in checks)
(OUT / "entitlement-request-evidence-checklist-check.md").write_text(f"""# Entitlement Request Evidence Checklist Check
