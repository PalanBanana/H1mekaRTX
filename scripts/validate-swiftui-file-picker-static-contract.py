...
SCHEMA = "h1mekartx.swiftui_file_picker_static_contract.v1"
...
def build_report(root: Path) -> dict[str, Any]:
    ...
    add_check(checks, "file_picker_view_exists", (root / "tools/host-app-no-runtime-swiftui/Sources/H1mekaRTXHostApp/LocalReportFilePickerView.swift").is_file(), "LocalReportFilePickerView.swift")
    ...
    return {
        ...
        "decision": "PASS_SWIFTUI_FILE_PICKER_STATIC_CONTRACT" if failed_count == 0 else "FAIL_SWIFTUI_FILE_PICKER_STATIC_CONTRACT",
        "passed_count": passed_count,
        "failed_count": failed_count,
        "checks": checks,
        "source_files": SOURCE_FILES,
        "optional_swift_build_probe": build_probe,
        "file_picker_static_contract_valid": failed_count == 0,
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# SwiftUI File-picker Static Contract Report",
            "",
            ...
            "",
            f"File-picker static contract valid: `{report['file_picker_static_contract_valid']}`",
            "",
            f"Actual app code continues: `{report['actual_app_code_continues']}`",
            "",
            f"Local file only: `{report['local_file_only']}`",
            "",
            f"JSON only: `{report['json_only']}`",
            "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate SwiftUI file-picker static contract.")
    ...
    json_path = out_dir / "swiftui-file-picker-static-contract-report.json"
    md_path = out_dir / "swiftui-file-picker-static-contract-report.md"
