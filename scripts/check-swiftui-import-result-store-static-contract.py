...
SCHEMA = "h1mekartx.swiftui_import_result_store_static_contract_check.v1"

REQUIRED_FILES = [
    "scripts/validate-swiftui-import-result-store-static-contract.py",
    "scripts/check-swiftui-import-result-store-static-contract.py",
    "docs/metal/swiftui-import-result-store-static-contract.md",
]

REQUIRED_TERMS = [
    "PASS_SWIFTUI_IMPORT_RESULT_STORE_STATIC_CONTRACT",
    "import_result_store_static_contract_valid",
...
def build_report(root: Path, out_dir: Path) -> dict[str, Any]:
    ...
    validator_run = run_command(
        [
            "python3",
            str(root / "scripts" / "validate-swiftui-import-result-store-static-contract.py"),
            "--root",
            str(root),
            "--out-dir",
            str(out_dir),
    ...
    report_path = out_dir / "swiftui-import-result-store-static-contract-report.json"
    report = json.loads(report_path.read_text()) if report_path.exists() else {}

    add_check(checks, "contract_schema", report.get("schema") == "h1mekartx.swiftui_import_result_store_static_contract.v1", f"schema={report.get('schema')!r}")
    add_check(checks, "contract_decision", report.get("decision") == "PASS_SWIFTUI_IMPORT_RESULT_STORE_STATIC_CONTRACT", f"decision={report.get('decision')!r}")
    add_check(checks, "contract_failed_count_zero", report.get("failed_count") == 0, f"failed_count={report.get('failed_count')!r}")
    add_check(checks, "contract_valid", report.get("import_result_store_static_contract_valid") is True, f"value={report.get('import_result_store_static_contract_valid')!r}")
    ...
    for key in [
        ...
        "swiftui_source_only",
        "local_import_result_store_contract_only",
        "local_ui_state_only",
        "observable_store_only",
        "no_runtime",
    ...
    return {
        ...
        "repo_root": str(root),
        "decision": "PASS_SWIFTUI_IMPORT_RESULT_STORE_STATIC_CONTRACT_READY" if failed_count == 0 else "FAIL_SWIFTUI_IMPORT_RESULT_STORE_STATIC_CONTRACT",
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# SwiftUI Import Result Store Static Contract Check",
            "",
            ...
            *rows,
            ...
            "## Safety Boundary",
            "",
            "This check validates SwiftUI import result store source and an optional Swift build probe only. It adds no driver installation, no driver activation, no provider transition, no device ownership transition, no hardware-path code, and no RTX 5070 Metal runtime.",
            "",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check SwiftUI import result store static contract.")
    ...
    json_path = out_dir / "swiftui-import-result-store-static-contract-check.json"
    md_path = out_dir / "swiftui-import-result-store-static-contract-check.md"
