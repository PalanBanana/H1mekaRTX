...
manifest_path = ROOT / "tools/hackintosh/entitlement-request-package-consistency-gate.json"
doc_path = ROOT / "docs/hackintosh/entitlement-request-package-consistency-gate.md"
out_path = ROOT / "release-readiness/entitlement-request-package-consistency-gate.json"
out_md_path = ROOT / "release-readiness/entitlement-request-package-consistency-gate.md"
...
add(checks, "manifest_schema", bool(manifest and manifest.get("schema") == "h1mekartx.entitlement_request_package_consistency_gate.v1"), "manifest schema")
add(checks, "output_schema", bool(out and out.get("schema") == "h1mekartx.entitlement_request_package_consistency_gate_output.v1"), "output schema")
add(checks, "output_decision_pass", bool(out and out.get("decision") == "PASS_ENTITLEMENT_REQUEST_PACKAGE_CONSISTENCY_GATE_READY"), "output decision")
add(checks, "output_ready_true", bool(out and out.get("entitlement_request_package_consistency_gate_ready") is True), "ready")
...
for obj_name, obj in [("manifest", manifest), ("output", out)]:
    for field in ["rtx5070_target_retained", "consistency_gate_only"]:
...
for field in ["input_package_skeleton_pass", "input_checklist_pass", "input_checklist_check_pass", "request_notes_template_ready", "inputs_safe"]:
...
consistency = out.get("consistency", []) if isinstance(out, dict) else []
add(checks, "output_consistency_items_present", bool(len(consistency) >= 10), "consistency count")
add(checks, "output_consistency_all_pass", bool(consistency and all(i.get("status") == "PASS" for i in consistency)), "consistency statuses")
...
decision = "PASS_ENTITLEMENT_REQUEST_PACKAGE_CONSISTENCY_GATE_READY" if failed == 0 else "FAIL_ENTITLEMENT_REQUEST_PACKAGE_CONSISTENCY_GATE"

report = {
    "schema": "h1mekartx.entitlement_request_package_consistency_gate_check.v1",
...
(OUT / "entitlement-request-package-consistency-gate-check.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")

rows = "\n".join(f"| `{c['name']}` | {'PASS' if c['passed'] else 'FAIL'} | {c['detail']} |" for c in checks)
(OUT / "entitlement-request-package-consistency-gate-check.md").write_text(f"""# Entitlement Request Package Consistency Gate Check
