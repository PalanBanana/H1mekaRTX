...
SCHEMA = "h1mekartx.swiftui_import_result_store_ui_actions.v1"
...
def build_report(root: Path) -> dict[str, Any]:
    return {
        ...
        "repo_root": str(root),
        "target": TARGET,
        "decision": "SWIFTUI_IMPORT_RESULT_STORE_UI_ACTIONS_READY",
        "plain_answer": "SwiftUI host app can modify local import result store state through local UI controls only. RTX 5070 Metal runtime remains disabled.",
        "goal": "RTX 5070 Metal full graphics acceleration research path",
        "actual_app_code_continues": True,
        "local_store_ui_actions_ready": True,
        "local_ui_controls_enabled": True,
        ...
        "next_stage_recommendation": "Stage 63 should add a static contract checker for local store UI actions and optional Swift build probe. Keep all actions local-state-only and no-runtime.",
        "safety_boundary": {
            "read_only": True,
            "swiftui_source_only": True,
            "local_store_ui_actions_only": True,
            "local_ui_state_only": True,
            "local_ui_controls_only": True,
            "no_runtime": True,
...
def markdown_report(data: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            f"Local store UI actions ready: `{data['local_store_ui_actions_ready']}`",
            "",
            f"Local UI controls enabled: `{data['local_ui_controls_enabled']}`",
            "",
            ...
            *files,
            ...
            "## Safety Boundary",
            "",
            "This stage adds SwiftUI local store UI actions only. It adds no driver installation, no driver activation, no provider transition, no device ownership transition, no hardware-path code, and no RTX 5070 Metal runtime.",
            "",
            "## Next Stage",
            "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate SwiftUI import result store UI actions report.")
    ...
    json_path = out_dir / "swiftui-import-result-store-ui-actions-report.json"
    md_path = out_dir / "swiftui-import-result-store-ui-actions-report.md"
