...
SCHEMA = "h1mekartx.swiftui_layout_static_contract_check.v1"

REQUIRED_FILES = [
    "scripts/validate-swiftui-layout-static-contract.py",
    "scripts/check-swiftui-layout-static-contract.py",
    "docs/metal/swiftui-layout-static-contract.md",
]

REQUIRED_TERMS = [
    "PASS_SWIFTUI_LAYOUT_STATIC_CONTRACT",
    "layout_static_contract_valid",
    "optional_swift_build_probe",
    "navigation_stack_required",
    "scroll_layout_required",
    "shared_import_store_required",
    "metal_goal_banner_required",
...
def build_report(root: Path, out_dir: Path) -> dict[str, Any]:
    ...
    validator_run = run_command(
        [
            "python3",
            str(root / "scripts" / "validate-swiftui-layout-static-contract.py"),
            "--root",
            str(root),
            "--out-dir",
            str(out_dir),
    ...
    report_path = out_dir / "swiftui-layout-static-contract-report.json"
    report = json.loads(report_path.read_text()) if report_path.exists() else {}

    add_check(checks, "contract_schema", report.get("schema") == "h1mekartx.swiftui_layout_static_contract.v1", f"schema={report.get('schema')!r}")
    add_check(checks, "contract_decision", report.get("decision") == "PASS_SWIFTUI_LAYOUT_STATIC_CONTRACT", f"decision={report.get('decision')!r}")
    add_check(checks, "contract_failed_count_zero", report.get("failed_count") == 0, f"failed_count={report.get('failed_count')!r}")
    add_check(checks, "contract_valid", report.get("layout_static_contract_valid") is True, f"value={report.get('layout_static_contract_valid')!r}")
    add_check(checks, "navigation_stack_required", report.get("navigation_stack_required") is True, f"value={report.get('navigation_stack_required')!r}")
    add_check(checks, "scroll_layout_required", report.get("scroll_layout_required") is True, f"value={report.get('scroll_layout_required')!r}")
    add_check(checks, "shared_import_store_required", report.get("shared_import_store_required") is True, f"value={report.get('shared_import_store_required')!r}")
    add_check(checks, "metal_goal_banner_required", report.get("metal_goal_banner_required") is True, f"value={report.get('metal_goal_banner_required')!r}")
    add_check(checks, "runtime_boundary_summary_required", report.get("runtime_boundary_summary_required") is True, f"value={report.get('runtime_boundary_summary_required')!r}")
    ...
    return {
        ...
        "repo_root": str(root),
        "decision": "PASS_SWIFTUI_LAYOUT_STATIC_CONTRACT_READY" if failed_count == 0 else "FAIL_SWIFTUI_LAYOUT_STATIC_CONTRACT",
        ...
        "safety_boundary": {
            ...
            "layout_static_contract_only": True,
            "no_runtime": True,
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# SwiftUI Layout Static Contract Check",
            "",
            ...
            *rows,
            ...
            "## Safety Boundary",
            "",
            "This check validates SwiftUI layout source and an optional Swift build probe only. It adds no driver installation, no driver activation, no provider transition, no device ownership transition, no hardware-path code, and no RTX 5070 Metal runtime.",
            "",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check SwiftUI layout static contract.")
    ...
    json_path = out_dir / "swiftui-layout-static-contract-check.json"
    md_path = out_dir / "swiftui-layout-static-contract-check.md"
