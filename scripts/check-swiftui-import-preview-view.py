...
SCHEMA = "h1mekartx.swiftui_import_preview_view_check.v1"

REQUIRED_FILES = [
    "tools/host-app-no-runtime-swiftui/Sources/H1mekaRTXHostApp/ImportPreviewView.swift",
    "tools/host-app-no-runtime-swiftui/Sources/H1mekaRTXHostApp/ImportPreviewViewModel.swift",
    "tools/host-app-no-runtime-swiftui/Sources/H1mekaRTXHostApp/ContentView.swift",
    "scripts/generate-swiftui-import-preview-view-report.py",
    "scripts/check-swiftui-import-preview-view.py",
    "docs/metal/swiftui-import-preview-view.md",
]
...
REQUIRED_TERMS = [
    ...
    "ImportPreviewRow",
    "LocalStatusImportValidator.validate",
    "sample-imported-host-status",
    "import_preview_view_ready",
    "local_status_validation_display_only",
...
def run_report(root: Path, out_dir: Path) -> dict[str, Any]:
    proc = subprocess.run(
        [
            "python3",
            str(root / "scripts" / "generate-swiftui-import-preview-view-report.py"),
            "--root",
            str(root),
            "--out-dir",
            str(out_dir),
...
def build_report(root: Path, out_dir: Path) -> dict[str, Any]:
    ...
    add_check(checks, "content_view_embeds_preview", "ImportPreviewView(viewModel: .sample)" in content, "embedded" if "ImportPreviewView(viewModel: .sample)" in content else "missing")
    ...
    report_path = out_dir / "swiftui-import-preview-view-report.json"
    report = json.loads(report_path.read_text()) if report_path.exists() else {}

    add_check(checks, "report_schema", report.get("schema") == "h1mekartx.swiftui_import_preview_view.v1", f"schema={report.get('schema')!r}")
    add_check(checks, "report_decision", report.get("decision") == "SWIFTUI_IMPORT_PREVIEW_VIEW_READY", f"decision={report.get('decision')!r}")
    add_check(checks, "preview_ready", report.get("import_preview_view_ready") is True, f"value={report.get('import_preview_view_ready')!r}")
    add_check(checks, "display_only", report.get("local_status_validation_display_only") is True, f"value={report.get('local_status_validation_display_only')!r}")
    ...
    return {
        ...
        "repo_root": str(root),
        "decision": "PASS_SWIFTUI_IMPORT_PREVIEW_VIEW_READY" if failed_count == 0 else "FAIL_SWIFTUI_IMPORT_PREVIEW_VIEW",
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# SwiftUI Import Preview View Check",
            "",
            ...
            *rows,
            ...
            "## Safety Boundary",
            "",
            "This check validates SwiftUI import preview UI only. It adds no driver installation, no driver activation, no provider transition, no device ownership transition, no hardware-path code, and no RTX 5070 Metal runtime.",
            "",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check SwiftUI import preview view.")
    ...
    json_path = out_dir / "swiftui-import-preview-view-check.json"
    md_path = out_dir / "swiftui-import-preview-view-check.md"
