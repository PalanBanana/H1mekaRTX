...
SCHEMA = "h1mekartx.project_generator_output_allowlist_gate_check.v1"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check project generator output allowlist gate.")
    ...
    contract_path = root / "docs" / "driverkit" / "project-generator-output-allowlist-gate.md"
    allowlist_json_path = root / "tools" / "driverkit-xcode-layout" / "project-generator-dryrun" / "project-generator-output-allowlist.json"
    allowlist_md_path = root / "tools" / "driverkit-xcode-layout" / "project-generator-dryrun" / "project-generator-output-allowlist.md"
    ...
    checks = [
        ...
        make_check("allowlist_json_exists", allowlist_json_path.exists(), str(allowlist_json_path)),
        make_check("allowlist_markdown_exists", allowlist_md_path.exists(), str(allowlist_md_path)),
        make_check("dryrun_manifest_exists", manifest_json_path.exists(), str(manifest_json_path)),
    ]
    ...
    allowlist = read_json(allowlist_json_path)
    checks.append(make_check(
        "allowlist_schema_matches",
        bool(allowlist and allowlist.get("schema") == "h1mekartx.project_generator_output_allowlist.v1"),
        "h1mekartx.project_generator_output_allowlist.v1",
    ))
    checks.append(make_check(
        "allowlist_gate_only_true",
        bool(allowlist and allowlist.get("project_generator_output_allowlist_gate_only") is True),
        "project_generator_output_allowlist_gate_only=true",
    ))
    checks.append(make_check(
        "promotion_gate_only_true",
        bool(allowlist and allowlist.get("promotion_gate_only") is True),
        "promotion_gate_only=true",
    ))
    checks.append(make_check(
        "allowed_outputs_exact",
        sorted(allowlist.get("allowed_dryrun_output_paths", [])) == sorted(EXPECTED_ALLOWED) if allowlist else False,
        "allowed dry-run outputs exact match",
    ))
    checks.append(make_check(
        "forbidden_outputs_exact",
        sorted(allowlist.get("forbidden_real_project_output_paths", [])) == sorted(FORBIDDEN_PATHS) if allowlist else False,
        "forbidden real project outputs exact match",
    ))
    checks.append(make_check(
        "forbidden_existing_empty",
        allowlist.get("forbidden_real_project_outputs_existing") == [] if allowlist else False,
    ...
    for field in [
        ...
        checks.append(make_check(
            f"allowlist_{field}_false",
            bool(allowlist and allowlist.get(field) is False),
            f"{field}=false",
        ))

    passed_count = sum(1 for check in checks if check["passed"])
    failed_count = len(checks) - passed_count
    decision = "PASS_PROJECT_GENERATOR_OUTPUT_ALLOWLIST_GATE_READY" if failed_count == 0 else "FAIL_PROJECT_GENERATOR_OUTPUT_ALLOWLIST_GATE"
    ...
    json_path = out_dir / "project-generator-output-allowlist-gate-check.json"
    ...
    md = f"""# Project Generator Output Allowlist Gate Check
...
"""
    md_path = out_dir / "project-generator-output-allowlist-gate-check.md"
