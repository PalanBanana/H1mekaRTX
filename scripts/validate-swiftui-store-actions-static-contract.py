...
SCHEMA = "h1mekartx.swiftui_store_actions_static_contract.v1"
...
REQUIRED_TERMS = [
    ...
    "LocalStatusImportResult.accept",
...
def build_report(root: Path) -> dict[str, Any]:
    ...
    add_check(checks, "action_view_exists", (root / "tools/host-app-no-runtime-swiftui/Sources/H1mekaRTXHostApp/ImportResultStoreActionView.swift").is_file(), "ImportResultStoreActionView.swift")
    ...
    return {
        ...
        "decision": "PASS_SWIFTUI_STORE_ACTIONS_STATIC_CONTRACT" if failed_count == 0 else "FAIL_SWIFTUI_STORE_ACTIONS_STATIC_CONTRACT",
        "passed_count": passed_count,
        "failed_count": failed_count,
        "checks": checks,
        "source_files": SOURCE_FILES,
        "optional_swift_build_probe": build_probe,
        "store_actions_static_contract_valid": failed_count == 0,
        "actual_app_code_continues": True,
        "local_ui_actions_only": True,
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# SwiftUI Store Actions Static Contract Report",
            "",
            ...
            "",
            f"Store actions static contract valid: `{report['store_actions_static_contract_valid']}`",
            "",
            f"Actual app code continues: `{report['actual_app_code_continues']}`",
            "",
            f"Local UI actions only: `{report['local_ui_actions_only']}`",
            "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate SwiftUI store actions static contract.")
    ...
    json_path = out_dir / "swiftui-store-actions-static-contract-report.json"
    md_path = out_dir / "swiftui-store-actions-static-contract-report.md"
