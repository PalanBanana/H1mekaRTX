...
manifest_path = ROOT / "tools/hackintosh/redacted-export-inventory-consistency-gate.json"
doc_path = ROOT / "docs/hackintosh/redacted-export-inventory-consistency-gate.md"
out_path = ROOT / "release-readiness/redacted-export-inventory-consistency-gate.json"
out_md_path = ROOT / "release-readiness/redacted-export-inventory-consistency-gate.md"
...
add(checks, "manifest_schema", bool(manifest and manifest.get("schema") == "h1mekartx.redacted_export_inventory_consistency_gate.v1"), "manifest schema")
add(checks, "output_schema", bool(out and out.get("schema") == "h1mekartx.redacted_export_inventory_consistency_gate_output.v1"), "output schema")
add(checks, "output_decision_pass", bool(out and out.get("decision") == "PASS_REDACTED_EXPORT_INVENTORY_CONSISTENCY_GATE_READY"), "output decision")
add(checks, "output_ready_true", bool(out and out.get("redacted_export_inventory_consistency_gate_ready") is True), "ready")
add(checks, "output_inventory_present_true", bool(out and out.get("inventory_present") is True), "inventory")
add(checks, "output_inventory_count_matches_true", bool(out and out.get("inventory_count_matches") is True), "inventory")
add(checks, "output_inventory_all_exist_true", bool(out and out.get("inventory_all_exist") is True), "inventory")
add(checks, "output_inventory_all_relative_true", bool(out and out.get("inventory_all_relative") is True), "inventory")
add(checks, "output_inventory_forbidden_absent_true", bool(out and out.get("inventory_forbidden_absent") is True), "inventory")
add(checks, "output_inventory_all_not_copied_true", bool(out and out.get("inventory_all_not_copied") is True), "inventory")
add(checks, "output_inventory_sha256_present_true", bool(out and out.get("inventory_sha256_present") is True), "inventory")
...
for field in [
    "input_inventory_ledger_pass",
    "input_inventory_ledger_check_pass",
    "input_inventory_ledger_ready",
    "input_plan_consistency_gate_ready",
...
decision = "PASS_REDACTED_EXPORT_INVENTORY_CONSISTENCY_GATE_READY" if failed == 0 else "FAIL_REDACTED_EXPORT_INVENTORY_CONSISTENCY_GATE"

report = {
    "schema": "h1mekartx.redacted_export_inventory_consistency_gate_check.v1",
...
(OUT / "redacted-export-inventory-consistency-gate-check.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")

rows = "\n".join(f"| `{c['name']}` | {'PASS' if c['passed'] else 'FAIL'} | {c['detail']} |" for c in checks)
(OUT / "redacted-export-inventory-consistency-gate-check.md").write_text(f"""# Redacted Export Inventory Consistency Gate Check
...
- Next Gate: `phase63h-redacted-export-bundle-assembly-dry-run-manifest`
