...
manifest_path = ROOT / "tools/hackintosh/manual-entitlement-request-packet-export-checklist.json"
doc_path = ROOT / "docs/hackintosh/manual-entitlement-request-packet-export-checklist.md"
out_path = ROOT / "release-readiness/manual-entitlement-request-packet-export-checklist.json"
out_md_path = ROOT / "release-readiness/manual-entitlement-request-packet-export-checklist.md"
...
add(checks, "manifest_schema", bool(manifest and manifest.get("schema") == "h1mekartx.manual_entitlement_request_packet_export_checklist.v1"), "manifest schema")
add(checks, "output_schema", bool(out and out.get("schema") == "h1mekartx.manual_entitlement_request_packet_export_checklist_output.v1"), "output schema")
add(checks, "output_decision_pass", bool(out and out.get("decision") == "PASS_MANUAL_ENTITLEMENT_REQUEST_PACKET_EXPORT_CHECKLIST_READY"), "output decision")
add(checks, "output_ready_true", bool(out and out.get("manual_entitlement_request_packet_export_checklist_ready") is True), "ready")
...
for obj_name, obj in [("manifest", manifest), ("output", out)]:
    for field in ["rtx5070_target_retained", "manual_export_checklist_only"]:
...
for field in [
    "input_local_submission_readiness_gate_pass",
    "input_local_submission_readiness_gate_check_pass",
    "input_local_submission_readiness_ready",
    "input_pre_submission_packet_ledger_ready",
    "input_package_consistency_gate_ready",
...
decision = "PASS_MANUAL_ENTITLEMENT_REQUEST_PACKET_EXPORT_CHECKLIST_READY" if failed == 0 else "FAIL_MANUAL_ENTITLEMENT_REQUEST_PACKET_EXPORT_CHECKLIST"

report = {
    "schema": "h1mekartx.manual_entitlement_request_packet_export_checklist_check.v1",
...
(OUT / "manual-entitlement-request-packet-export-checklist-check.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")

rows = "\n".join(f"| `{c['name']}` | {'PASS' if c['passed'] else 'FAIL'} | {c['detail']} |" for c in checks)
(OUT / "manual-entitlement-request-packet-export-checklist-check.md").write_text(f"""# Manual Entitlement Request Packet Export Checklist Check
...
- Next Gate: `phase63b-redacted-manual-export-bundle-manifest`
