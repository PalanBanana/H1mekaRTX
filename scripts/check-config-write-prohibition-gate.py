...
SCHEMA = "h1mekartx.config_write_prohibition_gate_check.v1"
...
REQUIRED_CONTRACT_TOKENS = [
    ...
    "CONFIG_WRITE_PROHIBITION_GATE_ONLY: True",
    ...
    "CONFIG_WRITE_PROHIBITION_AUDIT_SCOPE",
    "CONFIG_WRITE_PROHIBITION_GATE_RULE",
...
def scan_file(path: Path, markers: list[str]) -> list[dict]:
    ...
    for marker in markers:
        if marker in text:
            hits.append({"path": str(path), "marker_family": "configuration_write"})
    return hits

def main() -> int:
    parser = argparse.ArgumentParser(description="Check PCI configuration write prohibition gate.")
    ...
    contract_path = root / "docs" / "driverkit" / "config-write-prohibition-gate.md"
    gate_path = root / "tools" / "driverkit-activation" / "config-write-prohibition-gate.json"
    ...
    checks = [
        ...
        make_check("config_write_gate_json_exists", gate_path.exists(), str(gate_path)),
    ...
    checks.append(make_check("gate_schema_matches", bool(gate and gate.get("schema") == "h1mekartx.config_write_prohibition_gate.v1"), "gate schema"))
    checks.append(make_check("gate_config_writes_forbidden", bool(gate and gate.get("configuration_writes_forbidden") is True), "configuration writes forbidden"))
    checks.append(make_check("gate_state_enforced", bool(gate and gate.get("config_write_prohibition_state") == "ENFORCED"), "ENFORCED"))
    ...
    checks.append(make_check("gate_configuration_writes_attempted_false", bool(gate and gate.get("configuration_writes_attempted") is False), "configuration_writes_attempted=false"))
    ...
    decision = "PASS_CONFIG_WRITE_PROHIBITION_GATE_READY" if failed_count == 0 else "FAIL_CONFIG_WRITE_PROHIBITION_GATE"
    ...
    report = {
        ...
        "classification": "CLASSIFICATION_CONFIG_WRITE_PROHIBITION_GATE",
        "secondary_classification": "CLASSIFICATION_BAR_MAPPING_PROHIBITION_GATE",
        "tertiary_classification": "CLASSIFICATION_STATIC_CONTRACT",
        "scope": "Phase 29 PCI configuration write prohibition gate",
        "config_write_prohibition_gate_only": True,
        "configuration_writes_forbidden": True,
        "bar_mapping_forbidden": True,
        "bar_mmio_mutation_forbidden": True,
    ...
    json_path = out_dir / "config-write-prohibition-gate-check.json"
    ...
    md = f"""# PCI Configuration Write Prohibition Gate Check
...
Phase 29 enforces PCI configuration write prohibition.

PCI configuration write policy remains forbidden until a future separately reviewed phase.
...
This phase adds PCI configuration write prohibition only. It does not prove provider match, activate or deactivate a System Extension, load a dext, open a provider, map BAR memory, mutate configuration space, mutate MMIO, submit GPU commands, or claim RTX 5070 UI compositor acceleration.
"""
    md_path = out_dir / "config-write-prohibition-gate-check.md"
