...
manifest_path = ROOT / "tools/hackintosh/redacted-export-bundle-dry-run-plan.json"
doc_path = ROOT / "docs/hackintosh/redacted-export-bundle-dry-run-plan.md"
out_path = ROOT / "release-readiness/redacted-export-bundle-dry-run-plan.json"
out_md_path = ROOT / "release-readiness/redacted-export-bundle-dry-run-plan.md"
...
add(checks, "manifest_schema", bool(manifest and manifest.get("schema") == "h1mekartx.redacted_export_bundle_dry_run_plan.v1"), "manifest schema")
add(checks, "output_schema", bool(out and out.get("schema") == "h1mekartx.redacted_export_bundle_dry_run_plan_output.v1"), "output schema")
add(checks, "output_decision_pass", bool(out and out.get("decision") == "PASS_REDACTED_EXPORT_BUNDLE_DRY_RUN_PLAN_READY"), "output decision")
add(checks, "output_ready_true", bool(out and out.get("redacted_export_bundle_dry_run_plan_ready") is True), "ready")
add(checks, "output_include_files_exist_true", bool(out and out.get("include_files_exist") is True), "include files")
...
for obj_name, obj in [("manifest", manifest), ("output", out)]:
    for field in ["rtx5070_target_retained", "dry_run_plan_only"]:
...
decision = "PASS_REDACTED_EXPORT_BUNDLE_DRY_RUN_PLAN_READY" if failed == 0 else "FAIL_REDACTED_EXPORT_BUNDLE_DRY_RUN_PLAN"

report = {
    "schema": "h1mekartx.redacted_export_bundle_dry_run_plan_check.v1",
...
(OUT / "redacted-export-bundle-dry-run-plan-check.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")

rows = "\n".join(f"| `{c['name']}` | {'PASS' if c['passed'] else 'FAIL'} | {c['detail']} |" for c in checks)
(OUT / "redacted-export-bundle-dry-run-plan-check.md").write_text(f"""# Redacted Export Bundle Dry-Run Plan Check
