...
manifest_path = ROOT / "tools/hackintosh/entitlement-request-pre-submission-packet-ledger.json"
doc_path = ROOT / "docs/hackintosh/entitlement-request-pre-submission-packet-ledger.md"
out_path = ROOT / "release-readiness/entitlement-request-pre-submission-packet-ledger.json"
out_md_path = ROOT / "release-readiness/entitlement-request-pre-submission-packet-ledger.md"
...
add(checks, "manifest_schema", bool(manifest and manifest.get("schema") == "h1mekartx.entitlement_request_pre_submission_packet_ledger.v1"), "manifest schema")
add(checks, "output_schema", bool(out and out.get("schema") == "h1mekartx.entitlement_request_pre_submission_packet_ledger_output.v1"), "output schema")
add(checks, "output_decision_pass", bool(out and out.get("decision") == "PASS_ENTITLEMENT_REQUEST_PRE_SUBMISSION_PACKET_LEDGER_READY"), "output decision")
add(checks, "output_ready_true", bool(out and out.get("entitlement_request_pre_submission_packet_ledger_ready") is True), "ready")
add(checks, "output_fail_count_zero", bool(out and out.get("fail_count") == 0), "fail_count")
add(checks, "output_blocked_count_at_least_four", bool(out and out.get("blocked_count", 0) >= 4), "blocked_count")
add(checks, "output_placeholder_count_at_least_four", bool(out and out.get("placeholder_count", 0) >= 4), "placeholder_count")

for obj_name, obj in [("manifest", manifest), ("output", out)]:
    for field in ["rtx5070_target_retained", "pre_submission_ledger_only"]:
...
for field in [
    "input_consistency_gate_pass",
    "input_consistency_gate_check_pass",
    "input_consistency_gate_ready",
    "input_package_skeleton_ready",
...
decision = "PASS_ENTITLEMENT_REQUEST_PRE_SUBMISSION_PACKET_LEDGER_READY" if failed == 0 else "FAIL_ENTITLEMENT_REQUEST_PRE_SUBMISSION_PACKET_LEDGER"

report = {
    "schema": "h1mekartx.entitlement_request_pre_submission_packet_ledger_check.v1",
...
(OUT / "entitlement-request-pre-submission-packet-ledger-check.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")

rows = "\n".join(f"| `{c['name']}` | {'PASS' if c['passed'] else 'FAIL'} | {c['detail']} |" for c in checks)
(OUT / "entitlement-request-pre-submission-packet-ledger-check.md").write_text(f"""# Entitlement Request Pre-Submission Packet Ledger Check
