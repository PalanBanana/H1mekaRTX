...
SCHEMA = "h1mekartx.swiftui_local_status_model_loader_check.v1"
...
REQUIRED_FILES = [
    ...
    "scripts/generate-swiftui-local-status-model-loader-report.py",
    "scripts/check-swiftui-local-status-model-loader.py",
    "docs/metal/swiftui-local-status-model-loader.md",
]
...
REQUIRED_TERMS = [
    ...
    "local_status_model_loader_ready",
    "bundled_sample_only",
...
def run_report(root: Path, out_dir: Path) -> dict[str, Any]:
    proc = subprocess.run(
        [
            "python3",
            str(root / "scripts" / "generate-swiftui-local-status-model-loader-report.py"),
            "--root",
            str(root),
            "--out-dir",
            str(out_dir),
...
def build_report(root: Path, out_dir: Path) -> dict[str, Any]:
    ...
    sample_path = root / "tools/host-app-no-runtime-swiftui/Sources/H1mekaRTXHostApp/Resources/sample-host-status.json"
    ...
    add_check(checks, "sample_source_local", sample.get("statusSource") == "LOCAL_BUNDLED_SAMPLE_ONLY", f"value={sample.get('statusSource')!r}")
    ...
    report_path = out_dir / "swiftui-local-status-model-loader-report.json"
    report = json.loads(report_path.read_text()) if report_path.exists() else {}

    add_check(checks, "report_schema", report.get("schema") == "h1mekartx.swiftui_local_status_model_loader.v1", f"schema={report.get('schema')!r}")
    add_check(checks, "report_decision", report.get("decision") == "SWIFTUI_LOCAL_STATUS_MODEL_LOADER_READY", f"decision={report.get('decision')!r}")
    add_check(checks, "loader_ready", report.get("local_status_model_loader_ready") is True, f"value={report.get('local_status_model_loader_ready')!r}")
    add_check(checks, "bundled_sample_only", report.get("bundled_sample_only") is True, f"value={report.get('bundled_sample_only')!r}")
    ...
    return {
        ...
        "repo_root": str(root),
        "decision": "PASS_SWIFTUI_LOCAL_STATUS_MODEL_LOADER_READY" if failed_count == 0 else "FAIL_SWIFTUI_LOCAL_STATUS_MODEL_LOADER",
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# SwiftUI Local Status Model Loader Check",
            "",
            ...
            *rows,
            ...
            "## Safety Boundary",
            "",
            "This check validates bundled local status loading only. It adds no driver installation, no driver activation, no provider transition, no device ownership transition, no hardware-path code, and no RTX 5070 Metal runtime.",
            "",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check SwiftUI local status model loader.")
    ...
    json_path = out_dir / "swiftui-local-status-model-loader-check.json"
    md_path = out_dir / "swiftui-local-status-model-loader-check.md"
