...
echo "== H1mekaRTX Phase 24 Dext Load / Provider Match Proof Schema Verification =="
...
echo "== 3. Run dext load / provider match proof schema checker =="
python3 "${ROOT_DIR}/scripts/check-dext-load-provider-match-proof-schema.py" \
  --root "${ROOT_DIR}" \
  --out-dir "${OUT_DIR}"
...
echo "== Phase 24 dext load / provider match proof schema verification passed =="
