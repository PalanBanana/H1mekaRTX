...
echo "== H1mekaRTX Phase 31 Firmware/Reset/Display Init Prohibition Gate Verification =="
...
echo "== 3. Run firmware/reset/display-init prohibition gate checker =="
python3 "${ROOT_DIR}/scripts/check-firmware-reset-display-init-prohibition-gate.py" \
  --root "${ROOT_DIR}" \
  --out-dir "${OUT_DIR}"
...
echo "== Phase 31 firmware/reset/display-init prohibition gate verification passed =="
