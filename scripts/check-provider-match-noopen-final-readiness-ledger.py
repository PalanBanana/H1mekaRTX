...
manifest_path = ROOT / "tools/hackintosh/provider-match-noopen-final-readiness-ledger.json"
doc_path = ROOT / "docs/hackintosh/provider-match-noopen-final-readiness-ledger.md"
out_path = ROOT / "release-readiness/provider-match-noopen-final-readiness-ledger.json"
out_md_path = ROOT / "release-readiness/provider-match-noopen-final-readiness-ledger.md"
...
add(checks, "manifest_schema", bool(manifest and manifest.get("schema") == "h1mekartx.provider_match_noopen_final_readiness_ledger.v1"), "manifest schema")
add(checks, "output_schema", bool(out and out.get("schema") == "h1mekartx.provider_match_noopen_final_readiness_ledger_output.v1"), "output schema")
add(checks, "output_decision_pass", bool(out and out.get("decision") == "PASS_PROVIDER_MATCH_NOOPEN_FINAL_READINESS_LEDGER_READY"), "output decision")

for obj_name, obj in [("manifest", manifest), ("output", out)]:
    for field in ["rtx5070_target_retained", "ledger_only"]:
...
for field in [
    ...
    "input_evidence_matrix_ready",
    "input_preflight_ready",
    "default_hardoptin_refusal_preserved",
    "noopen_evidence_promotable_next_planning_gate",
    "inputs_safe",
...
decision = "PASS_PROVIDER_MATCH_NOOPEN_FINAL_READINESS_LEDGER_READY" if failed == 0 else "FAIL_PROVIDER_MATCH_NOOPEN_FINAL_READINESS_LEDGER"

report = {
    "schema": "h1mekartx.provider_match_noopen_final_readiness_ledger_check.v1",
...
(OUT / "provider-match-noopen-final-readiness-ledger-check.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")

rows = "\n".join(f"| `{c['name']}` | {'PASS' if c['passed'] else 'FAIL'} | {c['detail']} |" for c in checks)
(OUT / "provider-match-noopen-final-readiness-ledger-check.md").write_text(f"""# Provider Match No-Open Final Readiness Ledger Check
