...
echo "== H1mekaRTX Phase 19 User Approval + Rollback Flow Verification =="
...
echo "== 3. Run user approval + rollback flow checker =="
python3 "${ROOT_DIR}/scripts/check-user-approval-rollback-flow.py" \
  --root "${ROOT_DIR}" \
  --out-dir "${OUT_DIR}"
...
echo "== Phase 19 user approval + rollback flow verification passed =="
