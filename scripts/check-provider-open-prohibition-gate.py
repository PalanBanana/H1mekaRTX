...
SCHEMA = "h1mekartx.provider_open_prohibition_gate_check.v1"
...
FORBIDDEN_PROVIDER_OPEN_MARKER_PARTS = [
    ("IOPCIDevice", ".open"),
    ("IOPCIDevice", "::Open"),
    ("IOPCIDevice", "Open"),
    ("provider", ".open"),
    ...
    ("provider", "Open("),
    ("IOService", "Open"),
    ("IOConnect", "CallMethod"),
    ("IOConnect", "MapMemory"),
    ("IOConnect", "SetNotificationPort"),
]

FORBIDDEN_PROVIDER_OPEN_MARKERS = ["".join(parts) for parts in FORBIDDEN_PROVIDER_OPEN_MARKER_PARTS]

FORBIDDEN_HARDWARE_ACCESS_MARKER_PARTS = [
    ("mapDevice", "Memory"),
    ("MapDevice", "Memory"),
...
FORBIDDEN_HARDWARE_ACCESS_MARKERS = ["".join(parts) for parts in FORBIDDEN_HARDWARE_ACCESS_MARKER_PARTS]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check provider open prohibition gate.")
    ...
    contract_path = root / "docs" / "driverkit" / "provider-open-prohibition-gate.md"
    gate_path = root / "tools" / "driverkit-activation" / "provider-open-prohibition-gate.json"
    ...
    checks = [
        ...
        make_check("provider_open_gate_json_exists", gate_path.exists(), str(gate_path)),
    ...
    checks.append(make_check("gate_schema_matches", bool(gate and gate.get("schema") == "h1mekartx.provider_open_prohibition_gate.v1"), "gate schema"))
    checks.append(make_check("gate_provider_open_forbidden", bool(gate and gate.get("provider_open_forbidden") is True), "provider_open_forbidden=true"))
    checks.append(make_check("gate_state_enforced", bool(gate and gate.get("provider_open_prohibition_state") == "ENFORCED"), "ENFORCED"))
    ...
    missing_scope = []
    provider_open_hits = []
    hardware_access_hits = []
    for rel in AUDIT_SCOPE:
        p = root / rel
        if not p.exists():
            missing_scope.append(rel)
            continue
        provider_open_hits.extend(scan_file(p, FORBIDDEN_PROVIDER_OPEN_MARKERS))
        hardware_access_hits.extend(scan_file(p, FORBIDDEN_HARDWARE_ACCESS_MARKERS))

    checks.append(make_check("audit_scope_files_exist", not missing_scope, ",".join(missing_scope)))
    checks.append(make_check("no_forbidden_provider_open_markers", not provider_open_hits, json.dumps(provider_open_hits, sort_keys=True)))
    checks.append(make_check("no_forbidden_hardware_access_markers", not hardware_access_hits, json.dumps(hardware_access_hits, sort_keys=True)))
    ...
    decision = "PASS_PROVIDER_OPEN_PROHIBITION_GATE_READY" if failed_count == 0 else "FAIL_PROVIDER_OPEN_PROHIBITION_GATE"
    ...
    report = {
        ...
        "secondary_classification": "CLASSIFICATION_PROVIDER_MATCH_CANDIDATE_SUMMARY_GATE",
        ...
        "ledger_ready_required_for_execute": True,
        "activation_execution_gate_decision": "BLOCK_EXECUTE",
        "provider_open_hits": provider_open_hits,
        "hardware_access_hits": hardware_access_hits,
        "provider_match_candidate_summary_state": "SUMMARY_ONLY",
        "provider_match_proof_state": "NOT_ATTEMPTED",
        "dext_load_proof_state": "NOT_ATTEMPTED",
        "real_gpu_command_execution_proof_state": "NOT_ATTEMPTED",
    ...
    json_path = out_dir / "provider-open-prohibition-gate-check.json"
    ...
    md = f"""# Provider Open Prohibition Gate Check
...
- Provider Open Prohibition State: `{report['provider_open_prohibition_state']}`
...
- Provider Open Hits: `{len(provider_open_hits)}`
- Hardware Access Hits: `{len(hardware_access_hits)}`
...
- IOPCIDevice Open Attempted: `{report['iopcidevice_open_attempted']}`
- IOService Open Attempted: `{report['ioservice_open_attempted']}`
...
"""
    md_path = out_dir / "provider-open-prohibition-gate-check.md"
