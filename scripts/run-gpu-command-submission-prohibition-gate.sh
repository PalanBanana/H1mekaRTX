...
echo "== H1mekaRTX Phase 30 GPU Command Submission Prohibition Gate Verification =="
...
echo "== 3. Run GPU command submission prohibition gate checker =="
python3 "${ROOT_DIR}/scripts/check-gpu-command-submission-prohibition-gate.py" \
  --root "${ROOT_DIR}" \
  --out-dir "${OUT_DIR}"
...
echo "== Phase 30 GPU command submission prohibition gate verification passed =="
