...
manifest_path = ROOT / "tools/hackintosh/redacted-entitlement-request-final-review-packet-index.json"
doc_path = ROOT / "docs/hackintosh/redacted-entitlement-request-final-review-packet-index.md"
out_path = ROOT / "release-readiness/redacted-entitlement-request-final-review-packet-index.json"
out_md_path = ROOT / "release-readiness/redacted-entitlement-request-final-review-packet-index.md"
...
add(checks, "manifest_schema", bool(manifest and manifest.get("schema") == "h1mekartx.redacted_entitlement_request_final_review_packet_index.v1"), "manifest schema")
add(checks, "output_schema", bool(out and out.get("schema") == "h1mekartx.redacted_entitlement_request_final_review_packet_index_output.v1"), "output schema")
add(checks, "output_decision_pass", bool(out and out.get("decision") == "PASS_REDACTED_ENTITLEMENT_REQUEST_FINAL_REVIEW_PACKET_INDEX_READY"), "output decision")
add(checks, "output_ready_true", bool(out and out.get("redacted_entitlement_request_final_review_packet_index_ready") is True), "ready")
add(checks, "output_index_entry_count_10", bool(out and out.get("index_entry_count") == 10), "entry count")
add(checks, "output_index_all_exist_true", bool(out and out.get("index_all_exist") is True), "index")
add(checks, "output_index_all_relative_true", bool(out and out.get("index_all_relative") is True), "index")
add(checks, "output_index_all_decisions_ok_true", bool(out and out.get("index_all_decisions_ok") is True), "index")
add(checks, "output_index_all_sha256_present_true", bool(out and out.get("index_all_sha256_present") is True), "index")
add(checks, "output_index_forbidden_absent_true", bool(out and out.get("index_forbidden_absent") is True), "index")
...
decision = "PASS_REDACTED_ENTITLEMENT_REQUEST_FINAL_REVIEW_PACKET_INDEX_READY" if failed == 0 else "FAIL_REDACTED_ENTITLEMENT_REQUEST_FINAL_REVIEW_PACKET_INDEX"

report = {
    "schema": "h1mekartx.redacted_entitlement_request_final_review_packet_index_check.v1",
...
(OUT / "redacted-entitlement-request-final-review-packet-index-check.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")

rows = "\n".join(f"| `{c['name']}` | {'PASS' if c['passed'] else 'FAIL'} | {c['detail']} |" for c in checks)
(OUT / "redacted-entitlement-request-final-review-packet-index-check.md").write_text(f"""# Redacted Entitlement Request Final Review Packet Index Check
