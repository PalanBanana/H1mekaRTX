...
echo "== H1mekaRTX Phase 17 Activation Prerequisites Ledger Verification =="
...
echo "== 3. Run activation prerequisites ledger checker =="
python3 "${ROOT_DIR}/scripts/check-activation-prerequisites-ledger.py" \
  --root "${ROOT_DIR}" \
  --out-dir "${OUT_DIR}"
...
echo "== Phase 17 activation prerequisites ledger verification passed =="
