...
SCHEMA = "h1mekartx.swiftui_local_status_model_loader.v1"
...
def build_report(root: Path) -> dict[str, Any]:
    return {
        ...
        "repo_root": str(root),
        "target": TARGET,
        "decision": "SWIFTUI_LOCAL_STATUS_MODEL_LOADER_READY",
        "plain_answer": "SwiftUI host app can load a bundled local status model. It still performs no runtime driver or hardware work.",
        "goal": "RTX 5070 Metal full graphics acceleration research path",
        "actual_app_code_continues": True,
        "local_status_model_loader_ready": True,
        "bundled_sample_only": True,
        ...
        "next_stage_recommendation": "Stage 56 should add a local status import plan or validator. Do not add driver runtime, provider transition, device ownership transition, or hardware-path code yet.",
        "safety_boundary": {
            "read_only": True,
...
def markdown_report(data: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            f"Local status model loader ready: `{data['local_status_model_loader_ready']}`",
            "",
            f"Bundled sample only: `{data['bundled_sample_only']}`",
            "",
            ...
            *files,
            ...
            "## Safety Boundary",
            "",
            "This stage adds a bundled local status model loader only. It adds no driver installation, no driver activation, no provider transition, no device ownership transition, no hardware-path code, and no RTX 5070 Metal runtime.",
            "",
            "## Next Stage",
            "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate SwiftUI local status model loader report.")
    ...
    json_path = out_dir / "swiftui-local-status-model-loader-report.json"
    md_path = out_dir / "swiftui-local-status-model-loader-report.md"
