...
SCHEMA = "h1mekartx.signed_extension_packaging_plan_resolver_fixture_matrix.v1"

RESOLVER = "scripts/resolve-signed-extension-packaging-plan.py"

FIXTURES = [
    {
        "name": "sample_incomplete",
        "path": "packaging-plan/signed-extension-packaging-plan.sample.json",
        "expected_decision": "NOT_READY_SIGNED_EXTENSION_PACKAGING_EVIDENCE_REQUIRED",
        "expected_gate": "NO_GO_SIGNED_EXTENSION_PACKAGING_EVIDENCE_INCOMPLETE",
        "expected_ready": False,
        "expected_runtime_allowed": False
    },
    {
        "name": "redacted_ready",
        "path": "packaging-plan/signed-extension-packaging-plan.redacted-ready.fixture.json",
        "expected_decision": "PACKAGING_EVIDENCE_PRESENT_FOR_MANUAL_REVIEW_NO_RUNTIME",
        "expected_gate": "GO_PACKAGING_EVIDENCE_PRESENT_FOR_MANUAL_REVIEW_NO_RUNTIME",
        "expected_ready": True,
        "expected_runtime_allowed": False
    },
    {
        "name": "runtime_requested_negative",
        "path": "packaging-plan/signed-extension-packaging-plan.runtime-requested.fixture.json",
        "expected_decision": "NOT_READY_SIGNED_EXTENSION_PACKAGING_EVIDENCE_REQUIRED",
        "expected_gate": "NO_GO_SIGNED_EXTENSION_PACKAGING_EVIDENCE_INCOMPLETE",
        "expected_ready": False,
        "expected_runtime_allowed": False
...
def run_resolver(root: Path, plan: str, out_dir: Path) -> dict[str, Any]:
    proc = subprocess.run(
        [
            "python3",
            str(root / RESOLVER),
...
def build_report(root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="h1mekartx-stage74-") as tmp:
        tmp_path = Path(tmp)
        ...
        for fixture in FIXTURES:
            ...
            result = {
                ...
                "packaging_ready_for_manual_review": report.get("packaging_ready_for_manual_review"),
                "runtime_allowed_after_packaging_resolver": report.get("runtime_allowed_after_packaging_resolver"),
            ...
            add_check(checks, f"{fixture['name']}:ready", result["packaging_ready_for_manual_review"] is fixture["expected_ready"], f"actual={result['packaging_ready_for_manual_review']!r}, expected={fixture['expected_ready']!r}")
            add_check(checks, f"{fixture['name']}:runtime_disabled", result["runtime_allowed_after_packaging_resolver"] is False, f"value={result['runtime_allowed_after_packaging_resolver']!r}")
    ...
    return {
        ...
        "repo_root": str(root),
        "decision": "PASS_SIGNED_EXTENSION_PACKAGING_PLAN_RESOLVER_FIXTURE_MATRIX" if failed_count == 0 else "FAIL_SIGNED_EXTENSION_PACKAGING_PLAN_RESOLVER_FIXTURE_MATRIX",
        ...
        "next_stage_recommendation": "Stage 75 should add a combined entitlement-plus-packaging gate summary while keeping runtime disabled.",
        "safety_boundary": {
            "read_only": True,
    ...
    for item in report["fixtures"]:
        fixture_rows.append(
            "| `{name}` | `{decision}` | `{gate}` | `{ready}` | `{runtime}` |".format(
                ...
                gate=item["gate"],
                ready=item["packaging_ready_for_manual_review"],
                runtime=item["runtime_allowed_after_packaging_resolver"],
            )
        )

    return "\n".join(
        [
            "# Signed Extension Packaging Plan Resolver Fixture Matrix Report",
            "",
            ...
            *rows,
            ...
            "## Safety Boundary",
            "",
            "This stage validates signed-extension packaging plan resolver fixture matrix coverage only. It adds no driver installation, no driver activation, no provider transition, no device ownership transition, no hardware-path code, and no RTX 5070 Metal runtime.",
            "",
            "## Next Stage",
            "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate signed-extension packaging plan resolver fixture matrix.")
    ...
    json_path = out_dir / "signed-extension-packaging-plan-resolver-fixture-matrix-report.json"
    md_path = out_dir / "signed-extension-packaging-plan-resolver-fixture-matrix-report.md"
