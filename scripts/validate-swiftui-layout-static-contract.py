...
SCHEMA = "h1mekartx.swiftui_layout_static_contract.v1"
...
def build_report(root: Path) -> dict[str, Any]:
    ...
    add_check(checks, "content_uses_navigation_stack", "NavigationStack" in content, "present" if "NavigationStack" in content else "missing")
    add_check(checks, "content_uses_scroll_view", "ScrollView" in content, "present" if "ScrollView" in content else "missing")
    add_check(checks, "content_uses_shared_import_store", "private let importStore = LocalImportResultStore.sample" in content, "present" if "private let importStore = LocalImportResultStore.sample" in content else "missing")
    add_check(checks, "content_embeds_goal_banner", "MetalInjectionGoalBannerView()" in content, "embedded" if "MetalInjectionGoalBannerView()" in content else "missing")
    add_check(checks, "content_embeds_boundary_summary", "RuntimeBoundarySummaryView()" in content, "embedded" if "RuntimeBoundarySummaryView()" in content else "missing")
    add_check(checks, "content_embeds_store_view_shared", "ImportResultStoreView(store: importStore)" in content, "embedded" if "ImportResultStoreView(store: importStore)" in content else "missing")
    add_check(checks, "content_embeds_action_view_shared", "ImportResultStoreActionView(store: importStore)" in content, "embedded" if "ImportResultStoreActionView(store: importStore)" in content else "missing")
    ...
    return {
        ...
        "decision": "PASS_SWIFTUI_LAYOUT_STATIC_CONTRACT" if failed_count == 0 else "FAIL_SWIFTUI_LAYOUT_STATIC_CONTRACT",
        "passed_count": passed_count,
        "failed_count": failed_count,
        "checks": checks,
        "source_files": SOURCE_FILES,
        "optional_swift_build_probe": build_probe,
        "layout_static_contract_valid": failed_count == 0,
        "actual_app_code_continues": True,
        "navigation_stack_required": True,
        "scroll_layout_required": True,
        "shared_import_store_required": True,
        "metal_goal_banner_required": True,
        "runtime_boundary_summary_required": True,
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# SwiftUI Layout Static Contract Report",
            "",
            ...
            "",
            f"Layout static contract valid: `{report['layout_static_contract_valid']}`",
            "",
            f"Actual app code continues: `{report['actual_app_code_continues']}`",
            "",
            f"Navigation stack required: `{report['navigation_stack_required']}`",
            "",
            f"Scroll layout required: `{report['scroll_layout_required']}`",
            "",
            f"Shared import store required: `{report['shared_import_store_required']}`",
            "",
            f"Metal goal banner required: `{report['metal_goal_banner_required']}`",
            "",
            f"Runtime boundary summary required: `{report['runtime_boundary_summary_required']}`",
            "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate SwiftUI layout static contract.")
    ...
    json_path = out_dir / "swiftui-layout-static-contract-report.json"
    md_path = out_dir / "swiftui-layout-static-contract-report.md"
