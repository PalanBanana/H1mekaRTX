...
def build_report(root: Path, out_dir: Path) -> dict[str, Any]:
    ...
    add_check(checks, "content_view_embeds_action_view", "ImportResultStoreActionView(store: .sample)" in content, "embedded" if "ImportResultStoreActionView(store: .sample)" in content else "missing")
