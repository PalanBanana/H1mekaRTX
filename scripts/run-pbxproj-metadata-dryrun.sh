...
echo "== H1mekaRTX Phase 12 PBXProj Metadata Dry-Run Verification =="
...
echo "== 3. Generate pbxproj metadata dry-run outputs =="
python3 "${ROOT_DIR}/scripts/generate-pbxproj-metadata-dryrun.py" \
  --root "${ROOT_DIR}" \
  --out-dir "tools/driverkit-xcode-layout"

echo "== 4. Run pbxproj metadata dry-run checker =="
python3 "${ROOT_DIR}/scripts/check-pbxproj-metadata-dryrun.py" \
  --root "${ROOT_DIR}" \
  --out-dir "${OUT_DIR}"
...
echo "== Phase 12 pbxproj metadata dry-run verification passed =="
