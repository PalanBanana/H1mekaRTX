...
echo "== H1mekaRTX Phase 35 UI Frame Pacing / Latency Metric Schema Verification =="
...
echo "== 3. Run UI frame pacing / latency metric schema checker =="
python3 "${ROOT_DIR}/scripts/check-ui-frame-pacing-latency-metrics.py" \
  --root "${ROOT_DIR}" \
  --out-dir "${OUT_DIR}"
...
echo "== Phase 35 UI frame pacing / latency metric schema verification passed =="
