...
manifest_path = ROOT / "tools/hackintosh/entitlement-request-package-skeleton.json"
doc_path = ROOT / "docs/hackintosh/entitlement-request-package-skeleton.md"
notes_path = ROOT / "docs/hackintosh/driverkit-entitlement-request-notes-template.md"
out_path = ROOT / "release-readiness/entitlement-request-package-skeleton.json"
out_md_path = ROOT / "release-readiness/entitlement-request-package-skeleton.md"
...
add(checks, "notes_template_exists", notes_path.exists(), str(notes_path))
add(checks, "output_exists", out_path.exists(), str(out_path))
add(checks, "output_md_exists", out_md_path.exists(), str(out_md_path))
add(checks, "manifest_schema", bool(manifest and manifest.get("schema") == "h1mekartx.entitlement_request_package_skeleton.v1"), "manifest schema")
add(checks, "output_schema", bool(out and out.get("schema") == "h1mekartx.entitlement_request_package_skeleton_output.v1"), "output schema")
add(checks, "output_decision_pass", bool(out and out.get("decision") == "PASS_ENTITLEMENT_REQUEST_PACKAGE_SKELETON_READY"), "output decision")

for obj_name, obj in [("manifest", manifest), ("output", out)]:
    for field in ["rtx5070_target_retained", "package_skeleton_only"]:
...
for field in [
    "input_handoff_ledger_pass",
    "input_handoff_ledger_check_pass",
    "input_handoff_ready",
    "inputs_safe",
    "entitlement_request_package_skeleton_ready",
]:
...
for token in [
    ...
    add(checks, "notes_contains_" + token.replace(" ", "_"), token in notes, token)
...
decision = "PASS_ENTITLEMENT_REQUEST_PACKAGE_SKELETON_READY" if failed == 0 else "FAIL_ENTITLEMENT_REQUEST_PACKAGE_SKELETON"

report = {
    "schema": "h1mekartx.entitlement_request_package_skeleton_check.v1",
...
(OUT / "entitlement-request-package-skeleton-check.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")

rows = "\n".join(f"| `{c['name']}` | {'PASS' if c['passed'] else 'FAIL'} | {c['detail']} |" for c in checks)
(OUT / "entitlement-request-package-skeleton-check.md").write_text(f"""# Entitlement Request Package Skeleton Check
