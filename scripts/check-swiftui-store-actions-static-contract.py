...
SCHEMA = "h1mekartx.swiftui_store_actions_static_contract_check.v1"

REQUIRED_FILES = [
    "scripts/validate-swiftui-store-actions-static-contract.py",
    "scripts/check-swiftui-store-actions-static-contract.py",
    "docs/metal/swiftui-store-actions-static-contract.md",
]

REQUIRED_TERMS = [
    "PASS_SWIFTUI_STORE_ACTIONS_STATIC_CONTRACT",
    "store_actions_static_contract_valid",
    "optional_swift_build_probe",
    "local_ui_actions_only",
    "local_ui_state_only",
...
def build_report(root: Path, out_dir: Path) -> dict[str, Any]:
    ...
    validator_run = run_command(
        [
            "python3",
            str(root / "scripts" / "validate-swiftui-store-actions-static-contract.py"),
            "--root",
            str(root),
            "--out-dir",
            str(out_dir),
    ...
    report_path = out_dir / "swiftui-store-actions-static-contract-report.json"
    report = json.loads(report_path.read_text()) if report_path.exists() else {}

    add_check(checks, "contract_schema", report.get("schema") == "h1mekartx.swiftui_store_actions_static_contract.v1", f"schema={report.get('schema')!r}")
    add_check(checks, "contract_decision", report.get("decision") == "PASS_SWIFTUI_STORE_ACTIONS_STATIC_CONTRACT", f"decision={report.get('decision')!r}")
    add_check(checks, "contract_failed_count_zero", report.get("failed_count") == 0, f"failed_count={report.get('failed_count')!r}")
    add_check(checks, "contract_valid", report.get("store_actions_static_contract_valid") is True, f"value={report.get('store_actions_static_contract_valid')!r}")
    add_check(checks, "local_ui_actions_only", report.get("local_ui_actions_only") is True, f"value={report.get('local_ui_actions_only')!r}")
    ...
    return {
        ...
        "repo_root": str(root),
        "decision": "PASS_SWIFTUI_STORE_ACTIONS_STATIC_CONTRACT_READY" if failed_count == 0 else "FAIL_SWIFTUI_STORE_ACTIONS_STATIC_CONTRACT",
        ...
        "safety_boundary": {
            ...
            "store_actions_contract_only": True,
            "no_runtime": True,
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# SwiftUI Store Actions Static Contract Check",
            "",
            ...
            *rows,
            ...
            "## Safety Boundary",
            "",
            "This check validates SwiftUI local store action source and an optional Swift build probe only. It adds no driver installation, no driver activation, no provider transition, no device ownership transition, no hardware-path code, and no RTX 5070 Metal runtime.",
            "",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check SwiftUI store actions static contract.")
    ...
    json_path = out_dir / "swiftui-store-actions-static-contract-check.json"
    md_path = out_dir / "swiftui-store-actions-static-contract-check.md"
